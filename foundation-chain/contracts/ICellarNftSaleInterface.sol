...
interface ICellarNftSaleInterface is IERC721Upgradeable{
    ...
    function mintItem(address to, uint256 tokenId, string memory tokenURI) external returns(uint256);
    
    function makeOrder(uint256 tokenId,uint256 sellingPrice) external  returns(uint256);
    
    function cancleOrder(uint256 tokenId) external  returns(bool);
    
    function cancleOrder(address operator,uint256 tokenId) external returns(bool);
    
    function updateOrder(uint256 tokenId,uint256 sellingPrice) external  returns(bool);
    
    function updateOrder(address owner,uint256 tokenId,uint256 sellingPrice) external returns(bool);
    
    function transferNFT(address buyer, uint256 tokenId)external returns(uint256,uint256);

    function getOrderSellingPrice(uint256 tokenId)external view returns(uint256);
    
    function getOrderFeePercent(uint256 tokenId)external view returns(uint16);
    
    function getNFTOwnerByTokenId(uint256 tokenId) external  view returns(address);

    function getSaleOrderIdByToken(uint256 tokenId)external view returns(uint256);

    function getCurrentMaxSaleOrderId()external view returns(uint256);

    function getOrganiser()external view returns(address);
