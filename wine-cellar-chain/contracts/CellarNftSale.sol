    ...
    ReentrancyGuardUpgradeable
{
    ...
    address private _royaltyOrganiser; //版税费获得者地址
    ...
    mapping(uint256 => Order) saleOrderList; //销售订单(汇总){orderId=>order}
    ...
    function setRoyaltyOrganiser(address royaltyOrganiser) public onlyOwner {
        _royaltyOrganiser = royaltyOrganiser;
    }

    function getRoyaltyOrganiser() public view returns (address) {
        return _royaltyOrganiser;
    }
        ...
        returns (uint256)
    {
        require(
            ...
            "SolarNFT: the caller is not owner nor approved"
        );
        require(
            !_isSaleNFTAvailable(tokenId),
            "SolarNFT: the nft of token has a selling order, won't repeat."
        );
    ...
    function getNextOrderId() internal  returns (uint256 orderId) 
     {  
       ...
       bytes32 result =  keccak256(abi.encodePacked("ERC721", _orderId));
       orderId=uint256(result);
     }  

     function getNextOrderId2() public returns (string memory orderId) 
     {  
        _orderIds.increment();
        uint256 _orderId = _orderIds.current();
       bytes32 result =  keccak256(abi.encodePacked("ERC721", _orderId));
       orderId=Strings.toString(uint256(result));
     }  
    ...
    function cancleOrder(address operator, uint256 tokenId)
        internal
        isCallerRoleOrOwner(operator, tokenId)
        nonReentrant
        returns (bool)
    {
        require(
            saleOrderList[sellingNFTSaleOrders[tokenId]].isSelling,
            "SolarNFT: the order is not exists."
        );
        ...
        require(
            ...
            "SolarNFT: You won't cancle the order.Try to own the access."
        );
        ...
        emit CancelOrder(operator,saleOrderList[sellingNFTSaleOrders[tokenId]].seller,tokenId, _orderId);
    ...
    function updateOrder(
        ...
        uint256 tokenId,
        uint256 sellingPrice
    ) internal isCallerRoleOrOwner(owner, tokenId) returns (bool) {
        require(
            saleOrderList[sellingNFTSaleOrders[tokenId]].isSelling,
            "SolarNFT: the order is not exists."
        );
        require(
            ...
            "SolarNFT: You are not the owner and won't update the order."
        );

        //更新订单数据
        saleOrderList[sellingNFTSaleOrders[tokenId]]
        ...
        emit UpdateOrder(
            ...
            owner,
            tokenId,
            sellingNFTSaleOrders[tokenId],
            sellingPrice,
            saleOrderList[sellingNFTSaleOrders[tokenId]].feePercent
        );
        ...
        returns (uint256, uint256)
    {
        ...
        SaleDetails memory _saleDetails = SaleDetails({
            ...
            purchasePrice: saleOrderList[sellingNFTSaleOrders[tokenId]]
                .sellingPrice,
            buyer: buyer,
            isSuccess: true
        });
    ...
    {
        require(
            sellingNFTSaleOrders[tokenId] > 0,
            "SolarNFT721:found no selling data"
        );
        require(
            saleOrderList[sellingNFTSaleOrders[tokenId]].orderId > 0,
            "SolarNFT721:the selling saleOrder has sth wrong"
        );

        orderId = saleOrderList[sellingNFTSaleOrders[tokenId]].orderId;
        seller = saleOrderList[sellingNFTSaleOrders[tokenId]].seller;
        sellingPrice = saleOrderList[sellingNFTSaleOrders[tokenId]]
            .sellingPrice;
        feePercent = saleOrderList[sellingNFTSaleOrders[tokenId]].feePercent;
    }
    ...
    {
        require(
            saleOrderList[orderId].orderId > 0,
            "SolarNFT721:the selling saleOrder has sth wrong"
        );

        tokenId = saleOrderList[orderId].tokenId;
        seller = saleOrderList[orderId].seller;
        sellingPrice = saleOrderList[orderId].sellingPrice;
        feePercent = saleOrderList[orderId].feePercent;
    }
    ...
    function getCurrentTrateId() public view returns (uint256) {
        return _tradeIds.current();
    }

    function getOrderId(uint256 tokenId) public view returns (uint256){
        return sellingNFTSaleOrders[tokenId];
    }
}
